...
public class clsshowcasehelper : MonoBehaviour {
	...
	void metactivate () {
		...
		switch (vargamaction) {
			case 1:
				...
				varbombtransform.rigidbody.AddForce(new Vector3(0,2,0), ForceMode.VelocityChange);
				varbombtransform.rigidbody.AddTorque(Random.Range(5, 20), 0, Random.Range(5, 20),ForceMode.VelocityChange);
				break;
			case 2:
			...
			case 4:
				...
				break;
			case 5:
				//lerpz kinematic drive
				StartCoroutine("metDriveRagdoll",3);
				break;
			case 6:
				//alien animation states
				animation["Balance"].wrapMode = WrapMode.Loop;
	...
	private IEnumerator metAsmRoutine() {
		...
		yield return new WaitForSeconds(animation["transition"].length);
		animation.CrossFade("Rise");
		animation.CrossFadeQueued("Balance");
	}
